...
class Limb_Widget (limb_ui.Ui_MainWindow , chain_widget.Chain_Widget , QMainWindow) :
    ...
    def parse_extra (self) :
        ...
        self.limb = self.rigType_edit.text ()
    ...
    def build_setup (self) :
        ...
        if self.limb in self.limb_modules :
            ...
            # 初始化关节数量，当为胸腔模块的绑定的时候关节数量为5，否则为3
            if self.limb == 'spine' :
                self.jnt_number = 5
            else :
                self.jnt_number = 3
            self.limb_rig = limb_class (self.side , self.name , self.jnt_number , self.direction , self.length ,
                                        self.jnt_parent , self.ctrl_parent)
            self.limb_rig.build_setup()


    # 根据返回的参数,创建绑定系统
    def build_rig (self) :
        # 创建绑定
        self.limb_rig.build_rig ()


    ## 删除绑定
    def delete_rig (self) :
        # 删除绑定
        self.limb_rig.delete_rig ()
