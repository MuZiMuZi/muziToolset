...
class Pipeline (object) :
    ...
    def make_undo (func) :
        ...
        def wrap (*args , **kwargs) :
            # 无论被装饰函数成功还是抛异常，都必须关闭 undo chunk。
    ...
    def create_set(object, set_name, set_parent=None):
        ...
        if cmds.objExists(set_name):
            if cmds.nodeType(set_name) != 'objectSet':
                cmds.warning(u'名称已被非 objectSet 节点占用: {}'.format(set_name))
                return None
        else:
        ...
        if set_parent:
            if cmds.objExists(set_parent):
                if cmds.nodeType(set_parent) != 'objectSet':
                    cmds.warning(u'父选择集名称已被非 objectSet 节点占用: {}'.format(set_parent))
                    return set_name
            else:
                cmds.sets(name=set_parent, empty=True)
    ...
    def create_dynamic_curve_driven () :
        ...
        for dynamic_curve_node in crv_list :
            ...
            cmds.select (dynamic_curve_node , r = True)
            # 根据曲线创建关节链条
            jnt_dict = jointUtils.Joint.create_joints_on_curve ()

            # 头发曲线做动力学化曲线。
            cmds.mel.eval ('MakeCurvesDynamic')
    ...
    def copy_surface_create_geo () :
        ...
        # 删除物体没有被选择的面
        cmds.delete (non_selected_skin_faces)
    ...
    def duplicate_model (source_model ,new_name = None ,parent = None) :
        ...
        if new_name :
            duplicate_model_name = cmds.rename (
        ...
        if parent :
            ...
            if cmds.objExists (parent) :
                duplicate_model_name = cmds.parent (
                    duplicate_model_name ,
                    parent
                ) [0]
