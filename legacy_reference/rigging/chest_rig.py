...
class Chest_Rig (ikfk_rig.IKFK_Rig) :
    ...
    def __init__ (self , bp_joints = None , jnt_parent = None , control_parent = None , mirror = False ,
        ...
        self.ribB_jnt_mirror = cmds.mirrorJoint (self.ribB_jnt , mirrorYZ = True , mirrorBehavior = True ,
                                                 searchReplace = ['_l_' , '_r_']) [0]
        self.ribA_jnt_grp = hierarchyUtils.Hierarchy.get_child_object (self.ribA_jnt , type = 'joint')
        self.ribB_jnt_grp = hierarchyUtils.Hierarchy.get_child_object (self.ribB_jnt , type = 'joint')
        self.ribA_jnt_grp_mirror = hierarchyUtils.Hierarchy.get_child_object (self.ribA_jnt_mirror , type = 'joint')
        self.ribB_jnt_grp_mirror = hierarchyUtils.Hierarchy.get_child_object (self.ribB_jnt_mirror , type = 'joint')
    ...
    def create_chest_rig (self) :
        # 创建胸腔的控制器
        chest_ctrl_obj = controlUtils.Control.create_ctrl (self.chest_ctrl , shape = 'ball' , radius = 13 ,
                                                           axis = 'Z+' ,
                                                           pos = self.chest_jnt ,
        ...
        # 创建一个相乘节点来连接胸腔的旋转
        sternum_mult_node = cmds.createNode ('multDoubleLinear' ,
                                             name = self.sternumPlvot_jnt.replace ('jnt_' , 'mult_'))
        ...
        ribA_ikhandle_node_mirror = cmds.ikHandle (name = self.ribA_jnt_mirror.replace ('jnt_' , 'ikhandle_') ,
                                                   ...
                                                   sticky = 'sticky' , solver = 'ikRPsolver' ,
