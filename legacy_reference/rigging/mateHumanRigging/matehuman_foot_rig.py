...
class Foot_Rig(matehuman_base_rig.Base_Rig) :
	...
	def create_foot_ik_rig(self) :
		...
		single_ikhandle_node = \
			cmds.ikHandle(name = self.ball_ik_chain.replace('ikjnt' , 'ikhandle') , startJoint = self.foot_ik_jnt ,
			              endEffector = self.ball_ik_chain ,
			              sticky = 'sticky' , solver = 'ikSCsolver' , setupForRPsolver = True)[0]
		
		self.foot_ik_outputlocal = self.foot_ik_jnt.replace('jnt' , 'Localoutput')
