...
-   :material-link-variant:{ .lg .middle } **绑定模型**
    ...
    创建 SkinCluster，并确认 Joint Influence。
...
!!! tip "修改前先保存"
    需要重建 SkinCluster、改 Joint 层级或调整模型时，先导出权重数据，避免不可逆地丢失当前结果。

## 常见问题

**模型没有正确变形**：先检查 SkinCluster、Influence 和 Joint 层级，再检查局部权重。

**新增 Joint 不起作用**：确认它已经进入 SkinCluster Influence，而不是只存在于 Skeleton 中。
