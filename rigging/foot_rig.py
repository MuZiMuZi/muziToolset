...
class Foot_Rig(leg_rig.Leg_Rig):
    ...
    def create_footIK_rig(self,side):
        ...
        footHeelPivot_ctrl_obj = controlUtils.Control.create_ctrl(footHeelPivot_ctrl, shape = 'ball', radius = 3, axis = 'X+',
                                                             pos = footHeelPivot_jnt, parent = ankle_ctrl.replace('ctrl_','output_'))
        ...
        footToePivot_ctrl_obj = controlUtils.Control.create_ctrl(footToePivot_ctrl, shape = 'ball', radius = 3,
                                                                  axis = 'X+',
                                                                  pos = footToePivot_jnt, parent = footHeelPivot_ctrl.replace('ctrl_','output_'))
        ...
        footInnPivot_ctrl_obj = controlUtils.Control.create_ctrl(footInnPivot_ctrl, shape = 'ball', radius = 3,
                                                                  axis = 'X+',
                                                                  pos = footInnPivot_jnt, parent = footToePivot_ctrl.replace('ctrl_','output_'))
        ...
        footOutPivot_ctrl_obj = controlUtils.Control.create_ctrl(footOutPivot_ctrl, shape = 'ball', radius = 3,
                                                                  axis = 'X+',
                                                                  pos = footOutPivot_jnt, parent = footInnPivot_ctrl.replace('ctrl_','output_'))

        ball_IK_handle = cmds.ikHandle(name = ball_IK_jnt.replace('jnt_','ikhandle_'), startJoint = ankle_IK_jnt, endEffector = ball_IK_jnt,
        ...
        ball_IK_ctrl_obj = controlUtils.Control.create_ctrl(ball_IK_ctrl, shape = 'cube', radius = 11,
                                                                  axis = 'X+',
                                                                  pos = ball_IK_jnt, parent = footOutPivot_ctrl.replace('ctrl_','output_'))

        toe_IK_ctrl = 'ctrl_{}_toeIk_001'.format(side)
        toe_IK_ctrl_obj = controlUtils.Control.create_ctrl(toe_IK_ctrl, shape = 'cube', radius = 6,
                                                                  axis = 'X+',
                                                                  pos = toe_IK_jnt, parent = footOutPivot_ctrl.replace('ctrl_','output_'))
        ...
        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'toeTap', niceName = u' 抬脚尖 ',keyable = True)
        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'heelTap', niceName = u' 抬脚跟 ',keyable = True)

        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'toeRoll', niceName = u' 沿着脚尖旋转 ', keyable = True)
        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'heelRoll',  niceName = u' 沿着脚跟旋转 ',keyable = True)

        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'toeSide',  niceName = u' 沿着脚尖滑动 ',keyable = True)
        cmds.addAttr(ankle_ctrl, attributeType = 'double', longName = 'heelSide', niceName = u' 沿着脚跟滑动 ', keyable = True)
