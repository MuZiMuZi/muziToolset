...
class Arm_Rig(ikfk_rig.IKFK_Rig) :
    ...
    def create_arm_rig(self) :
        ...
        scapula_ctrl_obj = controlUtils.Control.create_ctrl(scapula_ctrl ,
                                                            ...
                                                            pos = None ,
                                                            parent = clavicle_ctrl.replace('ctrl_' , 'output_'))
        ...
        clavicle_ctrl_mirror_obj = controlUtils.Control.create_ctrl(clavicle_ctrl_mirror , shape = 'clavicle' ,
                                                                    radius = 13 , axis = 'Z+' ,
                                                                    pos = self.clavicle_jnt.replace('_l_' , '_r_') ,
        ...
        scapula_ctrl_mirror_obj = controlUtils.Control.create_ctrl(scapula_ctrl_mirror ,
                                                                   shape = 'Cube' , radius = 6 , axis = 'Z+' ,
                                                                   pos = None ,
                                                                   parent = clavicle_ctrl_mirror.replace('ctrl_' ,
                                                                                                         'output_'))
        cmds.matchTransform(scapula_ctrl_mirror.replace('ctrl_' , 'zero_') , self.scapula_jnt.replace('_l_' , '_r_') ,
                            position = True)
        ...
        cmds.matchTransform(shrug_ctrl_mirror.replace('ctrl_' , 'zero_') , self.shrug_jnt.replace('_l_' , '_r_') ,
                            position = True)
        ...
        # 创建肩膀耸肩的目标约束
        cmds.aimConstraint(shrug_ctrl , clavicle_ctrl.replace('ctrl_' , 'driven_') , worldUpType = 2 ,
                           aimVector = (1 , 0 , 0) ,
