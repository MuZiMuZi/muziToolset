...
class Build_Rig(base_rig.Base_Rig):
    ...
    def build_rig(self):
        ...
        '''
        if self.chest_rig:
            chest = chest_rig.Chest_Rig(bp_joints = None, jnt_parent = None, control_parent = 'output_m_cog_001')
            chest.create_chest_rig()
        if self.spine_rig:
            spine = spine_rig.Spine_Rig(bp_joints = None, jnt_parent = None, control_parent = None, mirror = False)
            spine.create_spine_rig()

        if self.arm_rig:
            arm = arm_rig.Arm_Rig(bp_joints = None, jnt_parent = None, control_parent = 'output_l_scapula_001', mirror = cmds.getAttr(self.arm_rig + '.mirror'),
                                  ribbon =  cmds.getAttr(self.arm_rig + '.ribbonRig'),jnt_number = cmds.getAttr(self.arm_rig + '.ribbonJntNumber'),
                                  space_list = ['world', 'cog', 'chest'] )
            arm.create_arm_rig()

        if self.leg_rig:
            leg = leg_rig.Leg_Rig(bp_joints = None, jnt_parent = None, control_parent = None, mirror = cmds.getAttr(self.leg_rig + '.mirror'),
                                  ribbon =  cmds.getAttr(self.leg_rig + '.ribbonRig'),jnt_number = cmds.getAttr(self.leg_rig + '.ribbonJntNumber'),
                                  space_list = ['world', 'cog', 'spine'])
            leg.create_leg_rig()

        if self.neck_rig:
            neck = neck_rig.Neck_Rig(bp_joints = None, jnt_parent = None, control_parent = None, mirror = False)
            neck.create_neck_rig()

        if self.foot_rig:
            foot = foot_rig.Foot_Rig(bp_joints = None, jnt_parent = None, control_parent = None, mirror = True)
            foot.create_footIK_rig('l')
            foot.create_footIK_rig('r')

        if self.hand_rig:
            hand = hand_rig.Hand_Rig(bp_joints = None, jnt_parent = None, control_parent = None, mirror = True)
