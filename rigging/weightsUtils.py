...
class Weights(object):
    ...
    def save_skinWeights(self):
       ...
       将蒙皮几何体对象的权重保存到给定权重文件夹，权重将保存在对象短名称下，并附加存储其蒙皮影响的文件
        ...
        if not skin_node_Res:
            ...
            #导出蒙皮权重
            cmds.deformerWeights(self.skinWeights_FileName, path =  self.skinWeights_Path, export = True, deformer = skin_node)
    ...
    def load_skinWeights(self):
       ...
       从给定权重文件夹加载蒙皮几何体对象的权重将从与对象短名称匹配的文件名加载权重，并添加其他文件以获取其影响
        ...
        # 获取蒙皮权重
        cmds.deformerWeights(self.skinWeights_FileName, path = self.skinWeights_Path, im = True, deformer = skin_node)
