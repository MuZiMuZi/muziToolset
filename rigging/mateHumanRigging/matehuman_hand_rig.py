...
class Hand_Rig(matehuman_base_rig.Base_Rig) :
	...
	def create_hand_fk_rig(self) :
		...
		# 创建手指各模块的FK关节链
		thumbHand_fk_system = matehuman_ikfk_rig.FK_Rig(self.thumbHand_drv , self.joint_parent , self.hand_ctrl_grp , 2)
		thumbHand_fk_system._create_fk_chain_system()
		
		indexHand_fk_system = matehuman_ikfk_rig.FK_Rig(self.indexHand_drv , self.joint_parent , self.hand_ctrl_grp , 2)
		indexHand_fk_system._create_fk_chain_system()
		
		middleHand_fk_system = matehuman_ikfk_rig.FK_Rig(self.middleHand_drv , self.joint_parent , self.hand_ctrl_grp ,
		                                                 2)
		middleHand_fk_system._create_fk_chain_system()
		
		ringHand_fk_system = matehuman_ikfk_rig.FK_Rig(self.ringHand_drv , self.joint_parent , self.hand_ctrl_grp , 2)
		ringHand_fk_system._create_fk_chain_system()
		
		pinkyHand_fk_system = matehuman_ikfk_rig.FK_Rig(self.pinkyHand_drv , self.joint_parent , self.hand_ctrl_grp , 2)
		pinkyHand_fk_system._create_fk_chain_system()
...
#example
# l_hand = Hand_Rig(joint_parent , control_parent , 'l')
