...
class FK_Rig(matehuman_base_rig.Base_Rig) :
	...
	def fk_chain_rig(self) :
		...
		for jnt in self.drv_jnts :
			fk_ctrl = controlUtils.Control.create_mateHuman_ctrl(jnt , 'fkctrl' , shape = 'circle' ,
			                                                     radius = self.redius ,
			                                                     axis = 'Y+' ,
			                                                     pos = jnt ,
			                                                     parent = None)
...
class IK_Rig(matehuman_base_rig.Base_Rig) :
	...
	def ik_chain_rig(self , Y_value = 1) :
		...
		endIK_local_ctrl = controlUtils.Control.create_mateHuman_ctrl(self.drv_jnts[2] , 'ikLocalctrl' ,
		                                                              shape = 'local' ,
		                                                              radius = 10 ,
		                                                              axis = 'X-' ,
		                                                              pos = self.drv_jnts[2] , parent = None)
		...
		if self.stretch :
			...
			cmds.setAttr(mult_node + '.input2' , 0.5 * side_value * Y_value)
			...
			cmds.setAttr(mult_upper_disBtw_node + '.input2' , side_value * Y_value)
			...
			cmds.setAttr(mult_lower_disBtw_node + '.input2' , side_value * Y_value)
