	...
	def create_namespace(self) :
		...
		for i in range(3) :
			self.cheekbone_bpjnt_list.append(
					'bpjnt_{}_{}{}CheekBone_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.cheekbone_jnt_list.append(
					'jnt_{}_{}{}CheekBone_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.cheekbone_ctrl_list.append(
					'ctrl_{}_{}{}CheekBone_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
		# 法令纹层级名称整理，法令纹关节三个
		for i in range(3) :
			self.nasolabial_bpjnt_list.append(
					'bpjnt_{}_{}{}Nasolabial_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.nasolabial_jnt_list.append(
					'jnt_{}_{}{}Nasolabial_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.nasolabial_ctrl_list.append(
					'ctrl_{}_{}{}Nasolabial_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
		
		# 脸颊层级名称整理，脸颊关节两个
		for i in range(2) :
			self.cheek_bpjnt_list.append(
					'bpjnt_{}_{}{}Cheek_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.cheek_jnt_list.append(
					'jnt_{}_{}{}Cheek_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
			self.cheek_ctrl_list.append(
					'ctrl_{}_{}{}Cheek_{:03d}'.format(self.side , self.name , self._rtype , i + 1))
